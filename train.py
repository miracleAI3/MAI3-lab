...
if __name__ == "__main__":
    ...
    os.environ["MAI3_MY_TESTING"] = args.my_testing
    os.environ["MAI3_CTXLEN"] = str(args.ctx_len)
    os.environ["MAI3_HEAD_SIZE_A"] = str(args.head_size_a)
    if args.dim_att <= 0:
    ...
    if args.my_pile_stage > 0:
        ...
        if args.my_pile_stage >= 2:  # find latest saved model
            ...
            for p in os.listdir(args.proj_dir):
                if p.startswith("mai3") and p.endswith(".pth"):
                    p = ((p.split("-"))[1].split("."))[0]
                    if p != "final":
                        if p == "init":
                            p = -1
            ...
            if max_p == -1:
                args.load_model = f"{args.proj_dir}/mai3-init.pth"
            else:
                args.load_model = f"{args.proj_dir}/mai3-{max_p}.pth"
...
############################################################################
#
# MAI3-5 {args.precision.upper()} on {args.num_nodes}x{args.devices} {args.accelerator.upper()}, bsz {args.num_nodes}x{args.devices}x{args.micro_bsz}={args.real_bsz}, {args.strategy} {'with grad_cp' if args.grad_cp > 0 else ''}
#
...
"""
    ...
    os.environ["MAI3_FLOAT_MODE"] = args.precision
    ...
    os.environ["MAI3_JIT_ON"] = "1"
    if "deepspeed_stage_3" in args.strategy:
        os.environ["MAI3_JIT_ON"] = "0"
    ...
    from src.model import MAI3
    model = MAI3(args)

    if len(args.load_model) == 0 or args.my_pile_stage == 1:  # shall we build the initial weights?
        init_weight_name = f"{args.proj_dir}/mai3-init.pth"
